...
public class astar {
    ...
    String[] paths;
    val po;
    astar (val popp) throws IOException
    {
      po= new val(popp);
      pop = new mapnode();
      q = new ArrayList<>();
    ...
    {
        int indicator=5000, qa, fdis=0, eudis;
        test = new mapnode(po.map[hi][hj]);                
        if((Math.abs(po.map[i][j].curr.val-test.curr.val)) <= po.maxz)
          {     
              eudis = heu(po.map[i][j],po.map[hi][hj]);
              if(hi==i || hj==j)
              {
                  po.map[hi][hj].curr.dis = po.map[i][j].curr.dis+10+po.map[i][j].curr.val-test.curr.val+eudis ;                  
              }
              else if(hi!=i && hj!=j)
              {
                 po.map[hi][hj].curr.dis = po.map[i][j].curr.dis+14+po.map[i][j].curr.val-test.curr.val+eudis; 
              }   
    ...
    public int heu(mapnode s, mapnode t)
    {
        int eudis;
        eudis = (int) Math.sqrt((Math.pow(s.curr.x-t.curr.x,2)+Math.pow(s.curr.y-t.curr.y,2)+Math.pow(s.curr.val-t.curr.val,2)));
        return eudis;
    }
